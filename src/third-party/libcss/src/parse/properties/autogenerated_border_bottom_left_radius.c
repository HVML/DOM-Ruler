		...
		css_style *result)
{
	...
	if ((token->type == CSS_TOKEN_IDENT) && (lwc_string_caseless_isequal(token->idata, c->strings[INHERIT], &match) == lwc_error_ok && match)) {
			...
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_BORDER_BOTTOM_LEFT_RADIUS, 0,BORDER_BOTTOM_LEFT_RADIUS_AUTO);
	} else {
		css_fixed length = 0;
		uint32_t unit = 0;
		*ctx = orig_ctx;
		...
		error = css__stylesheet_style_appendOPV(result, CSS_PROP_BORDER_BOTTOM_LEFT_RADIUS, 0, BORDER_BOTTOM_LEFT_RADIUS_SET);
		if (error != CSS_OK) {
			*ctx = orig_ctx;
			return error;
		}
