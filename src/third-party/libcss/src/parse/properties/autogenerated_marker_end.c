		...
		css_style *result)
{
	...
	if ((token->type == CSS_TOKEN_IDENT) && (lwc_string_caseless_isequal(token->idata, c->strings[INHERIT], &match) == lwc_error_ok && match)) {
			error = css_stylesheet_style_inherit(result, CSS_PROP_MARKER_END);
	} else if ((token->type == CSS_TOKEN_IDENT) && (lwc_string_caseless_isequal(token->idata, c->strings[NONE], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_MARKER_END, 0,MARKER_END_NONE);
		...
		error = css__stylesheet_style_appendOPV(result, CSS_PROP_MARKER_END, 0, MARKER_END_URI);
		if (error != CSS_OK) {
			*ctx = orig_ctx;
			return error;
		}
