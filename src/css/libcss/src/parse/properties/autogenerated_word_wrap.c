		...
		css_style *result)
{
	...
	if ((lwc_string_caseless_isequal(token->idata, c->strings[INHERIT], &match) == lwc_error_ok && match)) {
			...
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_WORD_WRAP, 0,WORD_WRAP_NORMAL);
	} else if ((lwc_string_caseless_isequal(token->idata, c->strings[BREAK_WORD], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_WORD_WRAP, 0,WORD_WRAP_BREAK_WORD);
	} else {
		error = CSS_INVALID;
	}
