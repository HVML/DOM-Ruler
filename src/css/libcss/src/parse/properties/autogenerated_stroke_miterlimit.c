		...
		css_style *result)
{
	...
	if ((token->type == CSS_TOKEN_IDENT) && (lwc_string_caseless_isequal(token->idata, c->strings[INHERIT], &match) == lwc_error_ok && match)) {
			error = css_stylesheet_style_inherit(result, CSS_PROP_STROKE_MITERLIMIT);
	} else if ((token->type == CSS_TOKEN_IDENT) && (lwc_string_caseless_isequal(token->idata, c->strings[AUTO], &match) == lwc_error_ok && match)) {
			error = css__stylesheet_style_appendOPV(result, CSS_PROP_STROKE_MITERLIMIT, 0,STROKE_MITERLIMIT_AUTO);
		...
		error = css__stylesheet_style_appendOPV(result, CSS_PROP_STROKE_MITERLIMIT, 0, STROKE_MITERLIMIT_SET);
		if (error != CSS_OK) {
			*ctx = orig_ctx;
			return error;
		}
